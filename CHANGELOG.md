...
* [#125](https://github.com/readium/swift-toolkit/issues/125) Setting `window.location` from JavaScript in an EPUB resource no longer breaks the pagination. Navigations to publication resources are now performed as regular navigator jumps, external URLs are delegated with `navigator(_:presentExternalURL:)`, and navigations to unsafe URLs (e.g. `data:`) are blocked.
