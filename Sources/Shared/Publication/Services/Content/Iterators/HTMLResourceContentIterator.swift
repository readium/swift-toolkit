...
public actor HTMLResourceContentIterator: ContentIterator {
    ...
    private nonisolated func parse(document: Document, locator: Locator, beforeMaxLength: Int) throws -> ParsedElements {
        ...
        func head(_ node: Node, _ depth: Int) throws {
            if let node = node as? Element {
                ...
                if tag == "br" {
                    ...
                    // An `<svg>` wrapping a single `<image>`, as generated by
                    // some EPUB toolchains for cover pages is a plain image.
                    if let href = try node.wrappedImageHREFRelativeToHREF(baseHREF) {
...
/// Tags of the elements which don't draw anything by themselves, and therefore
...
/// containing it.
private func url(_ href: String, relativeTo baseHREF: AnyURL?) -> AnyURL? {
    AnyURL(string: href)
        .flatMap { baseHREF?.resolve($0) ?? $0 }
}
...
private extension Node {
    ...
    /// Returns the URL of the bitmap drawn by an `<svg>` wrapping a single
    /// `<image>`, as generated by some EPUB toolchains for cover pages.
    ///
    /// Returns nil for a richer SVG, which is a drawing of its own rather
    /// than a wrapper around a bitmap.
    ///
    /// The TypeScript counterpart is `findWrappedImage` in
    /// `Sources/Navigator/EPUB/Scripts/src/content.ts`, which must recognize
    ...
    func wrappedImageHREFRelativeToHREF(_ baseHREF: AnyURL?) throws -> AnyURL? {
        ...
        for child in getChildNodes().compactMap({ $0 as? Element }) {
            ...
            guard !nonRenderingSVGTags.contains(tag) else {
                continue
            }
            guard tag == "image", image == nil else {
                return nil
            }
            image = child
        }
    ...
    /// or its SVG 1.1 form, still used by most publications.
    ///
    /// The `xlink` prefix is matched as written rather than resolved against
    /// the namespace declarations, as SwiftSoup parses the document without
    /// any namespace support.
    func hrefOrXLinkHREF() throws -> String? {
        try attr("href").orNilIfBlank()
            ?? getAttributes()?
            .first { $0.getKey().hasSuffix(":href") }?
            .getValue()
            .orNilIfBlank()
    }
