...
class EPUBMetadataParserTests: XCTestCase {
    ...
    /// The EPUB 3 media overlay classes take precedence over the EPUB 2 ones.
    /// https://github.com/readium/swift-toolkit/issues/85
    func testParseMediaOverlaysClassesSupersedeEPUB2Metas() throws {
        let sut = try parseMetadata("media-overlays-precedence")
        XCTAssertEqual(sut.mediaOverlay?.activeClass, "-epub-media-overlay-active")
        XCTAssertEqual(sut.mediaOverlay?.playbackActiveClass, "-epub-media-overlay-playing")
    }
    ...
    /// An EPUB 2 `<meta name= content=>` tag is superseded by a structured element (EPUB 3
    /// `<meta property=>` or `<dc:x>`) with the same property.
    /// https://github.com/readium/swift-toolkit/issues/85
    func testLegacyMetaSupersededByStructuredElementInOtherMetadata() throws {
        ...
        XCTAssertEqual(sut.otherMetadata, [
            // The structured elements take precedence over the equivalent EPUB 2 metas,
            ...
            "http://purl.org/dc/terms/source": .string("DC source"),
