...
enum EPUBPositionsServiceTests {
    struct ReadingOrder {
        @Test func fromEmptyReadingOrder() async {
            let service = makeService(readingOrder: [])
            let result = await service.positionsByReadingOrder()
            #expect(result == .success([]))
        }

        @Test func fromReadingOrderWithOneResource() async {
        ...
        @Test func fromReadingOrderWithFewResources() async {
            let service = makeService(readingOrder: [
                (1, Link(href: "res"), nil),
        ...
        @Test func typeFallsBackOnHTML() async {
            let service = makeService(readingOrder: [
    ...
    struct Layouts {
        @Test func onePositionPerFixedLayoutResource() async {
        ...
        @Test func splitReflowableResourcesByProvidedLength() async {
            let service = makeService(
                layout: .reflowable,
                readingOrder: [
        ...
        @Test func layoutFallsBackToReflowable() async {
    ...
    struct MixedLayouts {
        @Test func reflowableAndFixedOverridesInFixedPublication() async {
            let service = makeService(
                layout: .fixed,
                readingOrder: [
        ...
        @Test func fixedAndReflowableOverridesInReflowablePublication() async {
            let service = makeService(
                layout: .reflowable,
                readingOrder: [
                    (60, Link(href: "chap1"), nil),
                    (20000, Link(href: "chap2", properties: makeProperties(layout: .fixed)), nil),
                    (60, Link(href: "chap3", properties: makeProperties(layout: .reflowable)), nil),
                ],
                reflowableStrategy: .archiveEntryLength(pageLength: 50)
            )
    ...
    struct ReflowableStrategy {
        @Test func archiveEntryLengthStrategy() async {
            let service = makeService(
                layout: .reflowable,
                readingOrder: [
...
private func makeService(
    layout: ReadiumShared.Layout? = nil,
    readingOrder: [(UInt64, Link, ArchiveProperties?)],
