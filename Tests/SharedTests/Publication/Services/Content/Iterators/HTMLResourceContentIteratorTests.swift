...
struct HTMLResourceContentIteratorTests {
    ...
    struct ExtendedDescriptions {
        ...
        @Test func inlineContainerTargetLinksToTheTargetItself() async throws {
            let elements = try await allElements("""
            <img src="a.jpg" alt="Chart" aria-details="chart-desc"/>
            <aside id="chart-desc"><p>A long description of the chart.</p></aside>
            """)

            let image = try #require(elements.first as? ImageContentElement)
            #expect(image.extendedDescriptions == [
                Link(href: "dir/res.xhtml#chart-desc"),
            ])
        }

        @Test func inlineContainerTargetTakesItsTitleFromItsAriaLabel() async throws {
            let elements = try await allElements("""
            <img src="a.jpg" alt="Chart" aria-details="chart-desc"/>
            <aside id="chart-desc" aria-label="Chart description"><p>Details.</p></aside>
            """)

            let image = try #require(elements.first as? ImageContentElement)
            #expect(image.extendedDescriptions == [
                Link(href: "dir/res.xhtml#chart-desc", title: "Chart description"),
            ])
        }

        @Test func anchorTargetLinksToItsDestinationTitledByItsText() async throws {
            let elements = try await allElements("""
            <img src="a.jpg" alt="Chart" aria-details="desc-link"/>
            <a id="desc-link" href="desc.xhtml">Extended description</a>
            """)

            let image = try #require(elements.first as? ImageContentElement)
            #expect(image.extendedDescriptions == [
                Link(href: "dir/desc.xhtml", title: "Extended description"),
            ])
        }

        @Test func anchorTargetPrefersItsAriaLabelAsTitle() async throws {
            let elements = try await allElements("""
            <img src="a.jpg" alt="Chart" aria-details="desc-link"/>
            <a id="desc-link" href="desc.xhtml" aria-label="Full description">ignored</a>
            """)

            let image = try #require(elements.first as? ImageContentElement)
            #expect(image.extendedDescriptions == [
                Link(href: "dir/desc.xhtml", title: "Full description"),
            ])
        }

        @Test func anchorTitleMixesTextAndNestedImageAltValues() async throws {
            let elements = try await allElements("""
            <img src="a.jpg" alt="Chart" aria-details="desc-link"/>
            <a id="desc-link" href="desc.xhtml">See
                the <img src="icon.png" alt="detailed"/>   <span>description</span></a>
            """)

            let image = try #require(elements.first as? ImageContentElement)
            #expect(image.extendedDescriptions == [
                Link(href: "dir/desc.xhtml", title: "See the detailed description"),
            ])
        }

        // MARK: HREF forms

        @Test func fragmentOnlyHREFResolvesAgainstTheResource() async throws {
            let elements = try await allElements("""
            <img src="a.jpg" alt="Chart" aria-details="desc-link"/>
            <a id="desc-link" href="#target">Description</a>
            """)

            let image = try #require(elements.first as? ImageContentElement)
            #expect(image.extendedDescriptions == [
                Link(href: "dir/res.xhtml#target", title: "Description"),
            ])
        }

        @Test func absoluteHTTPHREFPassesThroughUnchanged() async throws {
            let elements = try await allElements("""
            <img src="a.jpg" alt="Chart" aria-details="desc-link"/>
            <a id="desc-link" href="https://example.com/desc.html">Description</a>
            """)

            let image = try #require(elements.first as? ImageContentElement)
            #expect(image.extendedDescriptions == [
                Link(href: "https://example.com/desc.html", title: "Description"),
            ])
        }

        @Test func unsafeSchemesFallBackToTheInlineContainerBranch() async throws {
            let elements = try await allElements("""
            <img src="a.jpg" alt="Chart" aria-details="mail-link js-link"/>
            <a id="mail-link" href="mailto:author@example.com">Mail</a>
            <a id="js-link" href="javascript:alert(1)">Script</a>
            """)

            let image = try #require(elements.first as? ImageContentElement)
            #expect(image.extendedDescriptions == [
                Link(href: "dir/res.xhtml#mail-link"),
                Link(href: "dir/res.xhtml#js-link"),
            ])
        }

        @Test func anchorsWithNoUsableHREFAreTreatedAsInlineContainers() async throws {
            let elements = try await allElements("""
            <img src="a.jpg" alt="Chart" aria-details="no-href empty-href bare-hash"/>
            <a id="no-href">No href</a>
            <a id="empty-href" href="">Empty</a>
            <a id="bare-hash" href="#">Hash</a>
            """)

            let image = try #require(elements.first as? ImageContentElement)
            #expect(image.extendedDescriptions == [
                Link(href: "dir/res.xhtml#no-href"),
                Link(href: "dir/res.xhtml#empty-href"),
                Link(href: "dir/res.xhtml#bare-hash"),
            ])
        }

        // MARK: IDREF list

        @Test func multipleTargetsAreKeptInAttributeOrderAndDanglingIDsSkipped() async throws {
            let elements = try await allElements("""
            <img src="a.jpg" alt="Chart" aria-details="second missing first"/>
            <aside id="first"><p>First.</p></aside>
            <aside id="second"><p>Second.</p></aside>
            """)

            let image = try #require(elements.first as? ImageContentElement)
            #expect(image.extendedDescriptions == [
                Link(href: "dir/res.xhtml#second"),
                Link(href: "dir/res.xhtml#first"),
            ])
        }

        @Test func duplicateIDREFsToTheSameNodeCollapse() async throws {
            let elements = try await allElements("""
            <img src="a.jpg" alt="Chart" aria-details="chart-desc chart-desc"/>
        ...
        @Test func twoAnchorsWithTheSameDestinationBothSurvive() async throws {
            let elements = try await allElements("""
            <img src="a.jpg" alt="Chart" aria-details="link-a link-b"/>
            <a id="link-a" href="desc.xhtml">Summary</a>
            <a id="link-b" href="desc.xhtml">Full data table</a>
            """)

            let image = try #require(elements.first as? ImageContentElement)
            #expect(image.extendedDescriptions == [
                Link(href: "dir/desc.xhtml", title: "Summary"),
                Link(href: "dir/desc.xhtml", title: "Full data table"),
            ])
        }

        @Test func selfAndAncestorTargetsAreSkipped() async throws {
            let elements = try await allElements("""
            <figure id="the-figure">
                <img id="the-image" src="a.jpg" alt="Chart" aria-details="the-image the-figure real-desc"/>
                <figcaption>Caption</figcaption>
            </figure>
            <aside id="real-desc"><p>Details.</p></aside>
            """)

            let image = try #require(elements.compactMap { $0 as? ImageContentElement }.first)
            #expect(image.extendedDescriptions == [
                Link(href: "dir/res.xhtml#real-desc"),
            ])
        }

        @Test func allInvalidTargetsYieldNoAttributes() async throws {
            let elements = try await allElements("""
            <img id="the-image" src="a.jpg" alt="Chart" aria-details="missing the-image"/>
            """)
        ...
        @Test func ariaHiddenSuppressesExtendedDescriptions() async throws {
            let elements = try await allElements("""
            <img src="a.jpg" alt="Chart" aria-hidden="true" aria-details="chart-desc"/>
            <aside id="chart-desc"><p>Details.</p></aside>
            """)
        ...
        @Test func ariaDetailsCancelsAPresentationalRole() async throws {
            // `aria-details` is a global ARIA attribute, so it wins the
            // presentational-role conflict and the element stays in the
            // accessibility tree.
            let elements = try await allElements("""
            <img src="a.jpg" alt="Chart" role="presentation" aria-details="chart-desc"/>
            <aside id="chart-desc"><p>Details.</p></aside>
            """)

            let image = try #require(elements.first as? ImageContentElement)
            #expect(image.accessibleName == "Chart")
            #expect(image.extendedDescriptions == [
                Link(href: "dir/res.xhtml#chart-desc"),
            ])
        }
        ...
        @Test func noAriaDetailsYieldsNoExtendedDescriptions() async throws {
            let elements = try await allElements("""
