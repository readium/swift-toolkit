...
/// This is NOT a Promise implementation, because state-less and not thread-safe. It's only meant to flatten in-place a group of completion-based calls into a monadic chain.
///
/// eg.
...
final class Deferred<T> {
    ...
    typealias Completion<V> = (V?, Error?) -> Void
    ...
    /// }
    init(_ closure: @escaping (@escaping Completion<T>) throws -> Void) {
    ...
    ///     }
    /// }
    convenience init(_ closure: @escaping () throws -> Deferred<T>) {
    ...
    ///     }
    /// }
    convenience init(_ closure: @escaping (@escaping (T) -> Void, @escaping (Error) -> Void) throws -> Void) {
        self.init { completion in
    ...
    class func success(_ value: T) -> Deferred {
    ...
    func resolve(on queue: DispatchQueue = .main, _ completion: Completion<T>? = nil) {
    ...
    ///     }
    /// }
    func resolve(on queue: DispatchQueue = .main, _ completion: @escaping (Error?) -> Void) {
    ...
    ///    asyncOperation(val)
    /// }
    func flatMap<V>(_ transform: @escaping (T) throws -> Deferred<V>) -> Deferred<V> {
        return map(
    ...
    /// }
    func asyncMap<V>(_ transform: @escaping (T, Completion<V>) throws -> Void) -> Deferred<V> {
        return map(
    ...
    ///    throw error
    /// }
    func `catch`(_ recover: @escaping (Error) throws -> T) -> Deferred<T> {
        return map(
    ...
    func flatCatch(_ recover: @escaping (Error) throws -> Deferred<T>) -> Deferred<T> {
        return map(
            success: { val, compl in compl(val, nil) },
