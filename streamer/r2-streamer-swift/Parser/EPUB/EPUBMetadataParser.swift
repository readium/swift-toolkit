...
final class EPUBMetadataParser: Loggable {
    ...
    init(document: XMLDocument, displayOptions: XMLDocument?) {
    ...
    func parse() throws -> Metadata {
        ...
        if let source = parseMetadata(xpath: "dc:source") {
            otherMetadata["source"] = source
        }
        if let rights = parseMetadata(xpath: "dc:rights") {
    ...
    private func parseMetadata(xpath: String) -> Any? {
        guard let values = metadataElement?.xpath(xpath).map({ $0.stringValue }) else {
            return nil
        }
    ...
    private var rendition: EPUBRendition {
        func meta(_ property: String) -> String {
            return metadataElement?.firstChild(xpath: "opf:meta[@property='\(property)']")?.stringValue ?? ""
        }
    ...
    private func titleElement(for titleType: EPUBTitleType, from titles: NodeSet) -> XMLElement? {
        return titles.first {
            ...
            return metadataElement?
                .xpath("opf:meta[@refines='#\(eid)' and @property='title-type']")
                .contains { $0.stringValue == titleType.rawValue }
                ?? false
        }
    }
    ...
    private var modifiedDate: Date? {
        guard let iso8601DateString = metadataElement?.firstChild(xpath: "opf:meta[@property='dcterms:modified']")?.stringValue else {
            return nil
        }
    ...
    private func findContributorMetaElements() -> [XMLElement] {
        return metadataElement?.xpath("opf:meta[@property='dcterms:creator' or @property='dcterms:publisher' or @property='dcterms:contributor']").map { $0 } ?? []
    }
    ...
    private func parseContributor(from element: XMLElement, to metadata: inout Metadata) {
        guard var contributor = createContributor(from: element),
            let metadataElement = metadataElement else
        {
            return
        }
        ...
        if let eid = element.attr("id") {
            for roleElement in metadataElement.xpath("opf:meta[@refines='#\(eid)' and @property='role']") {
                contributor.roles.append(roleElement.stringValue)
            }
        }
    ...
    private func localizedString(for element: XMLElement?) -> LocalizedString? {
        ...
        if let elementId = element.attr("id") {
            ...
            for altScriptMeta in metadataElement.xpath("opf:meta[@refines='#\(elementId)' and @property='alternate-script']") {
                // If it have a value then add it to the translations dictionnary.
                let value = altScriptMeta.stringValue.trimmingCharacters(in: .whitespacesAndNewlines)
                guard !value.isEmpty, let lang = altScriptMeta.attr("lang") else {
                    continue
                }
                strings[lang] = value
            }
