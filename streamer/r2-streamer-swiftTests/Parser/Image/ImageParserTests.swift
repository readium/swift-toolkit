...
class ImageParserTests: XCTestCase {
    ...
    func testAcceptsCBZ() {
        XCTAssertNotNil(try parser.parse(file: cbzFile, fetcher: cbzFetcher))
    }
    
    func testAcceptsJPG() {
        XCTAssertNotNil(try parser.parse(file: jpgFile, fetcher: jpgFetcher))
    }
    ...
    func testReadingOrderIsSortedAlphabetically() throws {
        ...
        XCTAssertEqual(publication.readingOrder, [
            ...
            Link(href: "/Cory Doctorow's Futuristic Tales of the Here and Now/z-bc.jpg", type: "image/jpeg")
        ])
    }
